...
The two hooks :meth:`~pi_ml.loops.base.Loop.on_save_checkpoint` and :meth:`~pi_ml.loops.base.Loop.on_load_checkpoint` function very similarly to how LightningModules and Callbacks save and load state.
...
When the Trainer is restarting from a checkpoint (e.g., through :code:`trainer.fit(ckpt_path=...)`), the loop exposes a boolean attribute :attr:`~pi_ml.loops.base.Loop.restarting`.
...
For example, the implementation of the :meth:`~pi_ml.loops.base.Loop.reset` method could look like this given our previous example:
