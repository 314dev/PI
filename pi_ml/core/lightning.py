...
from pi_ml.callbacks.progress import base as progress_base
from pi_ml.core.hooks import CheckpointHooks, DataHooks, ModelHooks
from pi_ml.core.mixins import DeviceDtypeModuleMixin, HyperparametersMixin
from pi_ml.core.optimizer import LightningOptimizer
from pi_ml.core.saving import ModelIO
from pi_ml.trainer.connectors.logger_connector.fx_validator import _FxValidator
from pi_ml.utilities import _IS_WINDOWS, _TORCH_GREATER_EQUAL_1_10, GradClipAlgorithmType
from pi_ml.utilities.apply_func import apply_to_collection, convert_to_tensors
from pi_ml.utilities.cloud_io import get_filesystem
from pi_ml.utilities.distributed import distributed_available, sync_ddp
from pi_ml.utilities.exceptions import MisconfigurationException
from pi_ml.utilities.memory import get_model_size_mb
from pi_ml.utilities.model_summary import ModelSummary, summarize
from pi_ml.utilities.parsing import collect_init_args
from pi_ml.utilities.rank_zero import rank_zero_debug, rank_zero_deprecation, rank_zero_warn
from pi_ml.utilities.signature_utils import is_param_in_hook_signature
from pi_ml.utilities.types import _METRIC_COLLECTION, EPOCH_OUTPUT, LRSchedulerTypeUnion, STEP_OUTPUT
...
class LightningModule(
    ...
    def optimizers(
        ...
        Args:
            use_pl_optimizer: If ``True``, will wrap the optimizer(s) in a
                :class:`~pi_ml.core.optimizer.LightningOptimizer` for automatic handling of precision and
                profiling.
    ...
    def training_step(self, *args, **kwargs) -> STEP_OUTPUT:
        ...
        Args:
            ...
            hiddens (``Any``): Passed in if
                :paramref:`~pi_ml.core.lightning.LightningModule.truncated_bptt_steps` > 0.
    ...
    def predict_step(self, batch: Any, batch_idx: int, dataloader_idx: int = 0) -> Any:
        """Step function called during :meth:`~pi_ml.trainer.trainer.Trainer.predict`. By default, it
        calls :meth:`~pi_ml.core.lightning.LightningModule.forward`. Override to add any processing
        logic.

        The :meth:`~pi_ml.core.lightning.LightningModule.predict_step` is used
        to scale inference on multi-devices.

        To prevent an OOM error, it is possible to use :class:`~pi_ml.callbacks.BasePredictionWriter`
        callback to write the predictions to disk or database after each batch or on epoch end.

        The :class:`~pi_ml.callbacks.BasePredictionWriter` should be used while using a spawn
    ...
    def configure_callbacks(self) -> Union[Sequence[Callback], Callback]:
        ...
        Lightning will make sure :class:`~pi_ml.callbacks.model_checkpoint.ModelCheckpoint` callbacks
        run last.
        ...
        Note:
            Certain callback methods like :meth:`~pi_ml.callbacks.base.Callback.on_init_start`
    ...
    def configure_optimizers(self):
        ...
        ``self.log('metric_to_track', metric_val)`` in your :class:`~pi_ml.core.lightning.LightningModule`.
    ...
    def lr_scheduler_step(
        ...
        :class:`~pi_ml.trainer.trainer.Trainer` calls each scheduler.
    ...
    def optimizer_step(
        ...
        r"""
        Override this method to adjust the default way the
        :class:`~pi_ml.trainer.trainer.Trainer` calls each optimizer.
    ...
    def tbptt_split_batch(self, batch: Any, split_size: int) -> List[Any]:
        ...
        Note:
            ...
            :meth:`~pi_ml.callbacks.base.Callback.on_train_batch_start`
            if :paramref:`~pi_ml.core.lightning.LightningModule.truncated_bptt_steps` > 0.
    ...
    def summarize(self, max_depth: int = 1) -> ModelSummary:
        ...
        .. deprecated:: v1.5
            This method was deprecated in v1.5 in favor of `pi_ml.utilities.model_summary.summarize`
        ...
        rank_zero_deprecation(
            ...
            "Use `pi_ml.utilities.model_summary.summarize` instead.",
            stacklevel=6,
        )
    ...
    def get_progress_bar_dict(self) -> Dict[str, Union[int, str]]:
        ...
        .. deprecated:: v1.5
            ...
            `pi_ml.callbacks.progress.base.get_metrics` and will be removed in v1.7.
        ...
        **kwargs,
        ...
        Note:
            - Requires the implementation of the
              :meth:`~pi_ml.core.lightning.LightningModule.forward` method.
    ...
    def model_size(self) -> float:
        ...
        if not self._running_torchscript:  # remove with the deprecation removal
            rank_zero_deprecation(
                ...
                " Please use the `pi_ml.utilities.memory.get_model_size_mb`.",
                stacklevel=5,
            )
