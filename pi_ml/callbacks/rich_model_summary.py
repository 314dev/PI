...
from pi_ml.utilities.model_summary import get_human_readable_count
...
class RichModelSummary(ModelSummary):
    r"""
    Generates a summary of all layers in a :class:`~pi_ml.core.lightning.LightningModule`
    ...
    .. code-block:: python
        ...
        from pi_ml.callbacks import RichModelSummary

        trainer = Trainer(callbacks=RichModelSummary())

    You could also enable ``RichModelSummary`` using the :class:`~pi_ml.callbacks.RichProgressBar`

    .. code-block:: python

        from pi_ml import Trainer
        from pi_ml.callbacks import RichProgressBar
